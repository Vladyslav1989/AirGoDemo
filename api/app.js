 ...
 app.post ('/confirm',(req,res)=>{
   ...
   SmS.findOne({ token: req.body.number }, function (err, token) {
       ...
       User.findOne({ _id: token._userId, }, function (err, user) {
           ...
           if (user.isVerifiedPhone) return res.status(400).send({ type: 'already-verified', msg: 'This user has already been verified.' });
     ...
     user.save((err, registerUser)=>{
         if(err)
         {
            ...
            //res.status(200).send(registerUser) if we not using jwt token  we can send user object directly to MongoDb
                ...
                const data = { body: random_number, numbers:user.phoneNUmber};
                console.log(data)
            ...
            for (const property in user) {
             ...
             //////////////////////////////////////////////////////////////////////////////////////////////////////////////
                        // if we crete Doctor and we dont have any shedule object we beed to chek fro null and push
                            ...
                            console.log("here")
                                query={ "_id": useraPOIMNET.id }
                                update = { "$push": { "Shedule": { "_id": useraPOIMNET.date, "Booked": { "start":useraPOIMNET.start, "end":useraPOIMNET.end,"Client":useraPOIMNET.idC} }   }   }  ;
                                // weneed any kind of respond in oredr to close conection 
                                //console.log(update)
                                res.json(user)                
                            }
                            ...
                            for (i = 0; i < user.Shedule.length; i++) {
                                        ...
                                        if(useraPOIMNET.date.getTime()  === user.Shedule[i]._id.getTime()   ){
                                            ...
                                            //break;
                                                                    ...
                                                                    const pushUser =    Doctor.updateOne(query,update)
        ...
        console.log(doctorData)
         Client.findOne({"_id": useraPOIMNET.idC} , (error,user)=>{
            ...
            console.log(clientData)
             ...
             console.log(ter)
            let msg = `Dear ${clientData.firstName} this is a rimender taht you have an appoimnet  with Doctor ${doctorData.lastName}on ${useraPOIMNET.date.toISOString().split("T")[0]} at  ${req.body.startTime}`
